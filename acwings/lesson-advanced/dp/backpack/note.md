...
T487 金明的预算方案
- 依赖只形成1层的树型，还限制为2叉
- 状态计算时**按方案**来划分: 选1, 2, 1+2这种

有依赖的背包：依赖以任意普通树型出现，选择子结点就必须选择父节点。用上树型dp的方法

`dp[u][j]`
- 状态表示
  - 【集合】所有从以u为根的子树中选，切总题记不超过j的方案【属性】min
  - 相当于将原先**线性的i**变到了**树型下的u**，是1维dp的扩展
- 状态计算：
  - 对于`dp[u][]`，u本身肯定得选，但其子树不一定
  - 在每个子树中，可按**体积**划分（由于孩子数没有限制，那么按方案划分得话可能太大$2^i$），即用多大得体积
    - 则可以把每颗子树看成物品组，每个物品组内部有m个物品，第i个物品表示体积为i，价值为`dp[son(u)][i]` => 这样就是**分组背包问题**
    - tips: 按方案来划分，即类似深搜；而按体积划分则归类了方案数
